...
def build_orthomosaic(
    undistorted_sparse_dir: Path,
    undistorted_images_dir: Path,
    sfm_dir: Path,
    mosaic_path: Path,
    target_max_size_px: int = 8000,
    blend_mode: str = "feather",
    num_bands: int = 6,
    flow_refine: bool = True,
    flow_method: str = "farneback_slow",
    flow_downscale: float = 1.0,
    flow_max_px: float = 2.5,
    flow_smooth_ksize: int = 13,
    split_stripes: bool = False,
    stripe_threshold: float = 0.5,
    stripe_flow_max_px: float = 6.0,
    warp_model: str = "homography",
    apap_cell_size: int = 80,
    apap_sigma: float = 120.0,
    apap_min_weight: float = 1e-4,
    apap_regularization: float = 0.05,
    seam_cost: str = "color",
    ...
    u_axis = np.cross(n, a); u_axis /= np.linalg.norm(u_axis)
    ...
    print(
        "Plane fit:",
        f"pts={X.shape[0]}",
        f"inliers={int(inliers.sum())} ({inlier_ratio:.1%})",
        f"|height| mean={mean_abs_height:.4f}",
        f"p95={p95_abs_height:.4f}",
        f"max={max_abs_height:.4f}",
    )
    ...
    if debug_dir is not None:
        ...
        debug_dir.mkdir(parents=True, exist_ok=True)
        plane_stats = {
            "num_points3D": int(X.shape[0]),
            "plane_normal": n.tolist(),
            "plane_offset": float(d),
            "inliers": int(inliers.sum()),
            "inlier_ratio": float(inliers.sum() / len(inliers)),
            "height_stats": {
                "mean_abs": mean_abs_height,
                "p95_abs": p95_abs_height,
                "max_abs": max_abs_height,
            },
        }
        # Reprojection error statistics
        point_errors = np.array([p.error for _, p in rec.points3D.items()], dtype=float)
        if point_errors.size:
            plane_stats["reprojection_error_px"] = {
                "mean": float(point_errors.mean()),
                "median": float(np.median(point_errors)),
                "p90": float(np.percentile(point_errors, 90)),
                "max": float(point_errors.max()),
            }
        # Per-image residuals & camera distance to plane for quick QA
        ...
        for img in rec.images.values():
            ...
            if residuals:
                ...
                per_image_stats.append({
                    "image": img.name,
                    "points3D": int(res_arr.size),
                    "plane_residual_mean": float(res_arr.mean()),
                    "plane_residual_p95": float(np.percentile(res_arr, 95)),
                    "plane_residual_max": float(res_arr.max()),
                    "camera_distance_to_plane": float(dist),
                })
            else:
                per_image_stats.append({
                    "image": img.name,
                    "points3D": 0,
                    "plane_residual_mean": None,
                    "plane_residual_p95": None,
                    "plane_residual_max": None,
                    "camera_distance_to_plane": float(dist),
                })
    ...
    for _, img in undist_rec.images.items():
        ...
        plane = H_i2p @ corners; plane /= plane[2:3, :]
        plane_xy = plane[:2, :].T
        ...
        image_correspondences[name] = {
            "plane_raw": np.array(corr_plane, dtype=float) if corr_plane else np.zeros((0, 2), dtype=float),
    ...
    min_xy = all_xy.min(axis=0); max_xy = all_xy.max(axis=0)
    span = max_xy - min_xy
    ...
    # Sort images left-to-right to reduce seam accumulation
    img_entries.sort(key=lambda e: e["x_min"])

    use_multiband = (blend_mode == "multiband")
    flow_downscale = float(max(flow_downscale, 1e-3))
    blender = None
    if use_multiband:
        try:
    ...
    def _apap_warp_image(
        image_rgb: np.ndarray,
        H_img_to_mosaic: np.ndarray,
        corr_mosaic: np.ndarray,
        corr_image: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray, Dict[str, object]]:
        fallback_info: Dict[str, object] = {
            "mode": "homography_fallback",
            "roi": None,
            "corr_total": int(corr_mosaic.shape[0]) if corr_mosaic is not None else 0,
        }
        ...
        within = (
            (corr_mosaic[:, 0] >= roi_x0 - margin)
            & (corr_mosaic[:, 0] <= roi_x1 + margin)
            & (corr_mosaic[:, 1] >= roi_y0 - margin)
            & (corr_mosaic[:, 1] <= roi_y1 + margin)
        )
        ...
        for iy, gy in enumerate(grid_y_vals):
            for ix, gx in enumerate(grid_x_vals):
                ...
                if weights.sum() < max(apap_min_weight, 1e-6):
                    continue
        ...
        deviation_stats = {
            "mean": float(deviation[apap_pixels].mean()) if apap_pixels.any() else 0.0,
            "p90": float(np.percentile(deviation[apap_pixels], 90)) if apap_pixels.any() else 0.0,
            "max": float(deviation[apap_pixels].max()) if apap_pixels.any() else 0.0,
        }

        info: Dict[str, object] = {
            "mode": "apap",
            "roi": [roi_x0, roi_y0, roi_x1, roi_y1],
            "corr_total": int(corr_mosaic.shape[0]),
            "corr_used": int(corr_mosaic_local.shape[0]),
            "mesh_cells": int((grid_h - 1) * (grid_w - 1)),
            "apap_vertices": int(apap_used),
            "apap_pixel_ratio": float(apap_mask_roi.mean()),
            "deviation": deviation_stats,
        }

        return warped_full, mask_full, info

    # Prepare warps for optional seam finding
    warps, masks = [], []
    ...
    for entry in img_entries:
        ...
        if warp_model == "apap":
            ...
            if corr and "height" in corr:
                ...
                if heights.size:
                    apap_info["height_stats"] = {
                        "mean": float(np.mean(heights)),
                        "mean_abs": float(np.mean(np.abs(heights))),
                        "p90_abs": float(np.percentile(np.abs(heights), 90)),
                        "max_abs": float(np.max(np.abs(heights))),
                    }
        ...
        if mask.max() == 0:
            continue
        # Slightly erode to avoid thin halos
        try:
    ...
    if warp_model == "apap" and apap_diagnostics:
        ...
        for rec in apap_diagnostics:
            ...
            roi = rec.get("roi")
            print(
                f"  {image_label}: mode={mode}, corr_used={corr_used}, "
                f"mean_dev={mean_dev:.3f}px, max_dev={max_dev:.3f}px, roi={roi}"
            )
                ...
                stripe_groups = [
                    {
                        ...
                        "plane_u": float(plane_uv_map[image_names[idx]][0]),
                        "plane_v": float(plane_uv_map[image_names[idx]][1]),
                    }
                    for idx in range(len(image_names))
                ]

    def _measure_pair(gray_a: np.ndarray, gray_b: np.ndarray,
                      mask_a: np.ndarray, mask_b: np.ndarray,
                      name_a: str, name_b: str) -> Optional[Dict[str, object]]:
        ...
        return {
            ...
            "shift_x": shift_x,
            "shift_y": shift_y,
            "shift_mag": float(math.hypot(shift_x, shift_y)),
    ...
    def _compute_binary_seam(mask_a: np.ndarray, mask_b: np.ndarray, img_a: np.ndarray, img_b: np.ndarray,
                              gradient_weight: float = 0.0) -> Optional[Tuple[np.ndarray, np.ndarray, Dict[str, float]]]:
        ...
        # Ensure support only where original masks exist
    ...
    def _log_pair_metrics(label: str, metrics: List[Dict[str, object]]):
        ...
        print(
            f"{label}: pairs={len(metrics)}, "
            f"shift_mean={np.mean(shifts):.3f}px, "
            f"shift_p90={np.percentile(shifts, 90):.3f}px, "
            f"shift_max={np.max(shifts):.3f}px"
        )
        if maes.size:
            print(
                f"{label}: mae_mean={np.mean(maes):.3f}, "
                f"mae_p90={np.percentile(maes, 90):.3f}, mae_max={np.max(maes):.3f}"
            )
    ...
    if len(warps) >= 2:
        ...
        if not split_mode:
            ...
            if edges:
                ...
                # Anchor first image at zero offset
                A[-1, 0] = 1.0
                b_x[-1] = 0.0
                b_y[-1] = 0.0
            ...
            per_image_offsets = [
                {"image": image_names[idx], "offset_x": offsets[idx][0], "offset_y": offsets[idx][1]}
            ...
            for i, j, applied_x, applied_y, weight, response, overlap in edges:
                ...
                alignment_offsets.append({
                    "image_i": image_names[i],
                    "image_j": image_names[j],
                    "measured_shift_x": -applied_x,
                    "measured_shift_y": -applied_y,
                    "applied_offset_i": offsets[i],
                    "applied_offset_j": offsets[j],
                    "residual_x": residual_x,
                    "residual_y": residual_y,
                    "residual_mag": float(math.hypot(residual_x, residual_y)),
                    "weight": weight,
            ...
            per_image_offsets = [
                {"image": name, "offset_x": 0.0, "offset_y": 0.0}
    ...
    if alignment_offsets:
        ...
        print(
            "Alignment residuals:",
            f"mean={np.mean(residuals):.3f}px",
            f"p90={np.percentile(residuals, 90):.3f}px",
            f"max={np.max(residuals):.3f}px",
        )
    ...
    if split_mode:
        ...
        if len(group_warps) == 2:
            ...
            if res is not None:
                ...
                group_alignment_record = {
                    "measured_shift_x": res["shift_x"],
                    "measured_shift_y": res["shift_y"],
                    "applied_shift_x": dx,
                    "applied_shift_y": dy,
                    "shift_mag": res["shift_mag"],
                    "phase_corr_response": res["phase_corr_response"],
                    "overlap_px": res["overlap_px"],
                }
        ...
        if len(warps) == 2:
            seam_pair = _compute_binary_seam(
                group_masks[0],
                group_masks[1],
                group_warps[0],
                group_warps[1],
                gradient_weight=seam_gradient_weight if seam_cost == "gradient" else 0.0,
            )
    ...
    if seam_cost == "gradient" and len(warps) >= 2:
        for idx in range(len(warps) - 1):
            seam_pair = _compute_binary_seam(
                ...
                warps[idx],
                warps[idx + 1],
                gradient_weight=seam_gradient_weight,
            )
            ...
            seam_diag.update({
                "pair": [image_names[idx], image_names[idx + 1]],
                "mode": "adjacent",
                "cost_model": seam_cost,
            })
            seam_records.append(seam_diag)
    ...
    if seam_records:
        ...
        for rec in seam_records:
            ...
            print(
                f"  {pair[0]} ↔ {pair[1]} ({mode}): cost={seam_cost_val:.3f}, "
                f"grad_penalty={grad_cost_val:.3f}, overlap_px={overlap_px}"
            )

    # Local flow-based refinement in overlapping regions to reduce residual ghosting
    def refine_with_flow(ref_img, ref_mask, cur_img, cur_mask,
                         method: str = "farneback_slow",
                         max_px: float = 2.5,
                         smooth_ksize: int = 13):
        ...
        if abs(flow_downscale - 1.0) > 1e-3:
            ds = max(flow_downscale, 1e-3)
        ...
        if method == "dis" and hasattr(cv2, 'DISOpticalFlow_create'):
            ...
            # Slower but more stable Farneback
            pyr_scale = 0.5
            ...
            flow = cv2.calcOpticalFlowFarneback(
                cur_small, ref_small, None,
                pyr_scale, levels, winsize, iters, poly_n, 1.5, 0
            )
        else:
            # fallback
            flow = cv2.calcOpticalFlowFarneback(
        ...
        # smooth flow to avoid local artifacts
        try:
        ...
        flow_full[...,0] *= scale
        flow_full[...,1] *= scale
        # seam-aware weighting: stronger near seam, weaker deep inside either image
        ...
        wt = dt_cur / (dt_cur + dt_ref + 1e-6)
        wt = wt[..., None]
        flow_full *= wt
        # Build full-image flow field
        full_flow = np.zeros((cur_img.shape[0], cur_img.shape[1], 2), dtype=np.float32)
        full_flow[y:y+h, x:x+w, :] = flow_full
        # Apply only inside current mask (and mainly where overlapping)
    ...
    if flow_refine and len(warps) >= 2:
        ...
        for i in range(1, len(warps)):
            cur_img = warps[i]
            cur_mask = masks[i]
            cur_refined = refine_with_flow(ref_img, ref_mask, cur_img, cur_mask,
                                           method=flow_method, max_px=flow_max_for_refine, smooth_ksize=flow_smooth_ksize)
            refined.append(cur_refined)
        warps = refined

    pair_metrics_after: List[Dict[str, object]] = []
    if len(warps) >= 2:
        # Refresh grayscale representations after optional flow refinement
    ...
    if debug_dir is not None:
        diag = {
            "plane": plane_stats,
            "pairwise_alignment_before": pair_metrics_before,
            "pairwise_alignment_after": pair_metrics_after,
            "alignment_offsets": alignment_offsets,
            "image_offsets": per_image_offsets,
            "warp_model": warp_model,
            "seam_cost_model": seam_cost,
            "image_plane_uv": [
                {
                    "image": name,
                    "plane_u": float(plane_uv_map.get(name, (0.0, 0.0))[0]) if name in plane_uv_map else None,
                    "plane_v": float(plane_uv_map.get(name, (0.0, 0.0))[1]) if name in plane_uv_map else None,
                }
                for name in plane_uv_map.keys()
            ],
            "image_order": image_names,
            "original_image_order": original_image_names,
        ...
        if warp_model == "apap":
            diag["apap_params"] = {
                "cell_size": int(apap_cell_size),
                "sigma": float(apap_sigma),
                "min_weight": float(apap_min_weight),
                "regularization": float(apap_regularization),
            }
        if seam_cost == "gradient":
            diag["seam_params"] = {
                "gradient_weight": float(seam_gradient_weight),
            }
...
def main():
    p = argparse.ArgumentParser(description="Orthomosaic from COLMAP outputs")
    p.add_argument("--sfm", type=Path, required=True, help="Path to sparse model (cameras/images/points3D)")
    p.add_argument("--undist_sparse", type=Path, required=True)
    p.add_argument("--undist_images", type=Path, required=True)
    p.add_argument("--out", type=Path, required=True)
    p.add_argument("--max_mosaic_px", type=int, default=4000)
    p.add_argument("--blend", type=str, default="feather", choices=["feather", "multiband"])
    p.add_argument("--bands", type=int, default=6)
    p.add_argument("--split-stripes", action="store_true", help="Split images into plane stripes and mosaic separately")
    p.add_argument("--stripe-threshold", type=float, default=0.5, help="Minimum plane-v separation to trigger stripe split")
    p.add_argument("--stripe-flow-max-px", type=float, default=6.0, help="Max optical-flow magnitude when refining stripe mosaics")
    p.add_argument("--debug-dir", type=Path, default=None, help="Directory to save diagnostic outputs")
